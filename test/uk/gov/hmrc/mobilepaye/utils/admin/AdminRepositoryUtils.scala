...
package uk.gov.hmrc.mobilepaye.utils.admin
...
class AdminRepositoryUtils @Inject()(
  mongoComponent: MongoComponent
)(
  implicit ec: ExecutionContext
) extends AdminRepository(mongoComponent)(ec) {

  def insertFeatureFlag(name: FeatureFlagName, enabled: Boolean): Future[Boolean] =
    collection
      .insertOne(FeatureFlag(name = name, isEnabled = enabled))
